/***************************
1. anxiang.xiao@ragentek.com 2013-03-14 
 Description : fix bug MDWLYBLU-63 to make home menu  and back key to work  with vibrator 
 **********/
...
#ifdef GTP_KEY_USE_REPORT_POINT
static int tpd_keys_local[TPD_KEY_COUNT] = {KEY_MENU , KEY_HOMEPAGE , KEY_BACK};
static int tpd_keys_dim_local[TPD_KEY_COUNT][4] =  {{40,900,80,70},{120,900 ,80,70},{200,900,80,70}};
#endif
...
static s32 gtp_init_panel(struct i2c_client *client)
{
  ...
  u8 cfg_info_group4_charger[] = CTP_CFG_GROUP4_CHARGER;
    u8 *send_cfg_buf_charger[4] = {cfg_info_group1_charger, cfg_info_group2_charger, cfg_info_group3_charger,cfg_info_group4_charger};
#endif	
    ...
    GTP_DEBUG("len1=%d,len2=%d,len3=%d,len4=%d,get_len=%d", cfg_info_len[0], cfg_info_len[1], cfg_info_len[2], cfg_info_len[3],cfg_len);
   if ((!cfg_info_len[1]) && (!cfg_info_len[2]) && (!cfg_info_len[3]))
	...
	tp_sensor_id = rd_cfg_buf[GTP_ADDR_LENGTH];
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //u16 version_info;
#if GTP_KEY_USE_REPORT_KEY || GTP_KEY_USE_REPORT_POINT
    s32 idx = 0;
#endif
#ifdef TPD_PROXIMITY
...
static int touch_event_handler(void *unused)
{
	...
	printk ("\n@@@tp_sensor_id = %d@@@\n",tp_sensor_id);
      ...
      //  GTP_ERROR("lilonghui key_value", key_value);
#if GTP_KEY_USE_REPORT_KEY
        if (key_value || pre_key)
        {
            for (i = 0; i < TPD_KEY_COUNT; i++)
            {
                input_report_key(tpd->dev, tpd_keys_local[i], key_value & (0x01 << i));
   ...
   if (key_value|| pre_key)
  {
	...
	if (key_value != 0)
       ...
       // printk("\n ###@@@1433 x= %d, y = %d, touch_num= %d,key_value = %d  ### \n", input_x, input_y,touch_num,key_value);
	key_value = 0;
