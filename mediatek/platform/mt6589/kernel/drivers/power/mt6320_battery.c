...
#define RECHARGE_TEM_RANGE 5//pengjinlong add for re-charging
...
int g_rtc_soc = 0;//MDWLYBLU-332 pengjinlong add for batt_vol percent display
...
//CHAR_9_DISCHAR_1 pengjinlong change this for usb charging so slow in this file
#if defined(CHAR_9_DISCHAR_1)
int g_HW_stop_charging = 0;
#endif
int CHARGING_FULL_CURRENT=220;    // mA 
//MDWLYBLU-62 battert capacity display error when charging(start)
//pengjinlong change this ,close feature FUEL GAUGE
//int gForceADCsolution=0; 
int gForceADCsolution=1;
//MDWLYBLU-62 battert capacity display error when charging(end)
...
extern int get_rtc_spare_fg_value(void);//MDWLYBLU-332 pengjinlong add for batt_vol percent display
...
static void mt6320_battery_update(struct mt6320_battery_data *bat_data)
{
    ...
    {     
        if ( BMT_status.bat_exist )                
        {
            ...
            //pengjinlong modify for battery capaticy display full (start)
            if ( (BMT_status.bat_vol >= 4183) && (BMT_status.bat_full == KAL_TRUE) )
#endif            	
            {
                /*Use no gas gauge*/
                if( gForceADCsolution == 1 )
                {
                    bat_volt_check_point = (BMT_status.SOC += 1);
                    ...
                    xlog_printk(ANDROID_LOG_INFO, "###Power/Battery", "[Battery] BAT_FULL BMT_status.bat_vol:%d\r\n", BMT_status.bat_vol);
                    /* For user view */
		...
		//pengjinlong modify for battery capaticy display full(end)
                /*Use gas gauge*/
                else
                {                    
                    gSyncPercentage=1;
...
int g_Get_I_Charging(void)
{
    ...
    {
        ICharging = (ADC_I_SENSE - ADC_BAT_SENSE)*100/R_CURRENT_SENSE;//pengjinlong change R_CURRENT_SENSE to0.2 >  0.22Ohm
    }
    else
    {
        ICharging = 0;
...
int BAT_CheckBatteryStatus(void)
{
    ...
    if ( BMT_status.bat_charging_state != CHR_TOP_OFF)
    {
        ...
        //gADC_I_SENSE_offset = gADC_BAT_SENSE_temp - gADC_I_SENSE_temp;//wangmeng remove it for debug icharging
    }
...
/*
	MDWLYBLU-1042 pengjinlong change,first read battery voltage from get_hw_ocv()
	BMT_status.SOC depend on BMT_status.bat_vol after averaging
	(start)
*/
    /*Use no gas gauge*/
    if( gForceADCsolution == 1 )
...
#else        	
            if (BMT_status.bat_vol >= 4183)//RECHARGING_VOLTAGE) 
#endif    
            {               
                BMT_status.SOC = 100;    
                BMT_status.bat_full = KAL_TRUE;
                xlog_printk(ANDROID_LOG_INFO, "### chr_det == KAL_FALSE", "[BATTERY] BMT_status.bat_vol=%d\n\r", BMT_status.ADC_BAT_SENSE);          
                //xlog_printk(ANDROID_LOG_DEBUG, "###Power/Battery", "[BATTERY] upmu_is_chr_det() == KAL_FALSE .\n\r");
            }        
        }        
...
//        BMT_status.SOC = batterySOCSum / BATTERY_AVERAGE_SIZE;
 	...
 	if (((g_rtc_soc > BMT_status.SOC)&&(g_rtc_soc - BMT_status.SOC <20))
 		||((g_rtc_soc < BMT_status.SOC)&&( BMT_status.SOC - g_rtc_soc  <20))&&(g_rtc_soc >0))
 	{
 		if (g_rtc_soc > BMT_status.SOC){
 			if (upmu_is_chr_det() == KAL_FALSE)
 				BMT_status.SOC = (g_rtc_soc --);
 			else
 				BMT_status.SOC = g_rtc_soc;
 		}
 		else if (g_rtc_soc < BMT_status.SOC){
 			if (upmu_is_chr_det() == KAL_FALSE)
 				BMT_status.SOC = g_rtc_soc;
 			else
 				BMT_status.SOC = (g_rtc_soc ++);
...
#else
//MDWLYBLU-302  pengjinlong add for recharging (start)        
...
//MDWLYBLU-302  pengjinlong add for recharging (end) 
#endif
...
#endif
//MDWLYBLU-302  pengjinlong add for recharging (start)        
    if( (BMT_status.temperature <= (MAX_CHARGE_TEMPERATURE - RECHARGE_TEM_RANGE)) &&
	(BMT_status.temperature >= (MIN_CHARGE_TEMPERATURE + RECHARGE_TEM_RANGE)) &&
	(BMT_status.temperature != ERR_CHARGE_TEMPERATURE) &&
	(BMT_status.charger_protect_status == BATTERY_OVER_TEMP) )
    {
        BMT_status.bat_charging_state = CHR_PRE;
